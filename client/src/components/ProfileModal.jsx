...
const ProfileModal = ({setShowEdit}) => {
  ...
  return (
    <div className='fixed top-0 bottom-0 left-0 right-0 z-100 h-dcreen overflow-y-scroll bg-black/50'>
            ...
            <div className='bg-white rounded-lg shadow p-6'>
                ...
                <form className='space-y-4' onSubmit={handleSaveProfile}>
                    ...
                    <div className='flex flex-col items-start gap-3'>
                        <label htmlFor='profile_picture' className='block text-sm font-medium text-gray-700 mb-1 rounded'>
                            Profile Picture
                            ...
                            <div className='group/profile relative'>
                                <img src={editForm.profile_picture?URL.createObjectURL(editForm.profile_picture):user.profile_picture} className='w-24 h-24 rounded object-cover mt-2'/>
                                <div className='absolute hidden group-hover/profile:flex top-0 left-0 right-0 bottom-0 bg-black/20 rounded-full items-center jusitfy-center'>
                    ...
                    <div className='flex flex-col items-start gap-3'>
                        <label htmlFor='cover_photo' className='block text-sm font-medium text-gray-700 mb-1'>
                            ...
                            <div className='group/cover relative'>
                                ...
                                <div className='absolute hidden group-hover/cover:flex top-0 left-0 right-0 bottom-0 bg-black/20 rounded-lg items-cetner justify-center'>

                                    <Pencil className='w-5 h-5 text-white mt-2'/>
                                </div>
                            </div>
                    ...
                    <div>
                        ...
                        <input type='text' className='w-full p-3 border border-gray-200 rounded-lg' placeholder='enter full name' onChange={(e)=>setEditForm({...editForm,full_name:e.target.value})} value={editForm.full_name}/>
                    </div>
                    <div>
                        <label htmlFor='block text-sm font-medium text-gray-700 mb-1'>
                            Username
                        </label>
                        <input type='text' className='w-full p-3 border border-gray-200 rounded-lg' placeholder='enter user name' onChange={(e)=>setEditForm({...editForm,username:e.target.value})} value={editForm.username}/>
                    </div>
                    <div>
                        <label htmlFor='block text-sm font-medium text-gray-700 mb-1'>
                            Bio
                        </label>
                        <textarea rows={3} type='text' className='w-full p-3 border border-gray-200 rounded-lg' placeholder='enter bio' onChange={(e)=>setEditForm({...editForm,bio:e.target.value})} value={editForm.bio}/>
